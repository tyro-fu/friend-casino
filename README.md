...
面向朋友私下娱乐：私密房间（可选密码）、WebSocket 实时同步、SQLite 累计积分排行榜。不收集手机号，不接入第三方登录与支付。
...
- `积分统计规则`：若整段缺失，则使用默认 **大盲 10、每轮下注上限 100、小盲 5、初始筹码 2000、单桌 2～9 人**。
...
> **免费档须知**：`render.yaml` 已固定 `plan: free`，**不配置持久盘**。排行榜 / 房间密码在**重新部署**或**实例重建**后会丢失；实例 15 分钟无流量会**休眠**，再次访问需冷启动约 30-60 秒，属正常现象。
...
| 排行榜数据丢失 | 免费档无持久盘，重部署后 SQLite 文件会重建；如需保留数据建议升级付费档并挂载持久盘 |

## 部署到公网与免费/低成本主机建议

本项目是 **单进程 + WebSocket + 内存房间 + 本地 SQLite**，适合 **单实例常驻**；不适合多副本负载均衡（房间状态无法跨进程共享）。数据库文件需落在 **可持久化磁盘** 上，否则重启会丢排行榜与房间密码记录。
...
- **排行榜合并**：同一浏览器会复用本地 `poker_pid`；换浏览器或清除本地存储会视为新玩家。
...
| 积分与持久化 | `server/db.js`、`table.persistHandStats` |
